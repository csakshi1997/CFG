//
//  SeniorConsultantRefundVC.swift
//  CFGApp
//
...
class SeniorConsultantRefundVC: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func caseAction(sender: UIButton) {
        self.closedTableView?.isHidden = true
        self.pendingTableView?.isHidden = true
        if sender.tag == 1 {
            pendingBtn?.backgroundColor = UIColor(red: 229.0 / 255.0, green: 25.0 / 255.0, blue: 55.0 / 255.0, alpha: 1.0)
            pendingBtn?.setTitleColor(.white, for: .normal)
            ...
            ClosedBtn?.setTitleColor(.black, for: .normal)
            isPending = true
            print("Pending Maintenance Count: \(GlobalData.allPendingRefundModel.count)")
            if self.originalPendingData.count < 1 {
                self.noDataFoundVw?.isHidden = false
                self.pendingTableView?.isHidden = true
            } else {
            ...
            ClosedBtn?.setTitleColor(.white, for: .normal)
            isPending = false
            print("Pending Maintenance Count: \(GlobalData.allClosingRefundModel.count)")
            if self.originalClosedData.count < 1 {
                self.noDataFoundVw?.isHidden = false
                self.closedTableView?.isHidden = true
            } else {
                self.noDataFoundVw?.isHidden = true
                self.closedTableView?.isHidden = false
                self.closedTableView?.reloadData()
            }
        }
    }
}

extension SeniorConsultantRefundVC: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return isPending ? GlobalData.allPendingRefundModel.count : GlobalData.allClosingRefundModel.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if isPending {
            let cell: PendingSeniorConsultantCell = pendingTableView?.dequeueReusableCell(withIdentifier: "PendingSeniorConsultantCell", for: indexPath) as! PendingSeniorConsultantCell
            ...
            cell.doctorLblLabl?.text = pendingCaseData.doctor?.name
            cell.ClinicLabl?.text = pendingCaseData.clinic?.name
            cell.statusLabl?.text = pendingCaseData.status
            return cell
        } else {
            let cell: ClosingSeniorConsultantCell = closedTableView?.dequeueReusableCell(withIdentifier: "ClosingSeniorConsultantCell", for: indexPath) as! ClosingSeniorConsultantCell
            let closedCaseData = GlobalData.allClosingRefundModel[indexPath.row]
...
class PendingSeniorConsultantCell: UITableViewCell {
    @IBOutlet var vw: UIView?
    @IBOutlet var nameLabl: UILabel?
    @IBOutlet var patientNameLabl: UILabel?
    @IBOutlet var doctorLblLabl: UILabel?
...
class ClosingSeniorConsultantCell: UITableViewCell {
    @IBOutlet var vw: UIView?
