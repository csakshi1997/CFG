...
class CreateTriggerEventVC: UIViewController {
    @IBOutlet var initialView: UIView?
    @IBOutlet var patientNameView: UIView?
    @IBOutlet var detailView: UIView?
    @IBOutlet var triggerIdLabl: UILabel?
    @IBOutlet var eventNameLabl: UILabel?
    @IBOutlet var startDateTimeLabl: UILabel?
    @IBOutlet var endDateTimeLabl: UILabel?
    ...
    @IBOutlet var finalSubmitBtn: UIButton?
    ...
    override func viewDidLoad() {
        ...
        selectEventNameTxtFld?.delegate = self
    ...
    func setInitialUI() {
        ...
        patientNameView?.isHidden = true
        detailView?.layer.cornerRadius = 8
        submitBtn?.layer.masksToBounds = false
        submitBtn?.layer.cornerRadius = 8
        finalSubmitBtn?.layer.masksToBounds = false
        finalSubmitBtn?.layer.cornerRadius = 8
    ...
    @IBAction func submitAction() {
        ...
        patientNameView?.isHidden = false
    }
    
    @IBAction func finalSubmitAction() {
        guard let concernText = selectPatientNameTxtFld?.text, !concernText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            SceneDelegate.getSceneDelegate().window?.makeToast(PLEASE_SELECT_PATIENT_ID)
            return
        }
        
        let alert = UIAlertController(title: "Confirm Submission",
                                      message: "Do you really want to Create Trigger Event?",
...
extension CreateTriggerEventVC: UITextFieldDelegate {
